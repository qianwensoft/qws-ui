...
// 渲染字段
function renderField<TFormData>(
    ...
    case 'radio':
      return (
        <div className="form-radio-group">
          {options?.map((opt) => (
            <label key={opt.value} className="form-radio-wrapper">
              <input
                type="radio"
                name={String(name)}
                value={opt.value}
...
export function AdvancedForm<TFormData extends Record<string, any>>({
  ...
  const form = useForm<TFormData>({
    defaultValues: initialValues as TFormData,
  ...
  return (
    <div className={`advanced-form advanced-form-${layout} ${className}`}>
      <form
        onSubmit={(e) => {
          e.preventDefault();
          e.stopPropagation();
        ...
        <div className="form-fields">
          {fields.map((fieldConfig) => {
            ...
            if (fieldConfig.type === 'checkbox') {
              return (
                <form.Field
                  ...
                  validators={{
                    onChange: validateOnChange && fieldConfig.validate ? 
                      ({ value }: { value: any }) => fieldConfig.validate!(value) : 
                      undefined,
                    onBlur: validateOnBlur && fieldConfig.validate ? 
                      ({ value }: { value: any }) => fieldConfig.validate!(value) : 
                      undefined,
                  }}
                >
                  {(field) => (
                    <div className={`form-field form-field-checkbox ${fieldConfig.className || ''}`}>
            ...
            return (
              ...
              >
                {(field) => (
                  <div className={`form-field ${fieldConfig.className || ''}`}>
